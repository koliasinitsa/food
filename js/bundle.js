...
function calc() {
    ...
    function getDynamicInformation(selector) {
        ...
        input.addEventListener('input', () => {
            if (input.value.match(/\D/g)) {  // проверка на тип числовый тип
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
function tabs(tabsSelector, tabsContenctSelector, tabsParentSelector, activeClass) {
    const tabs = document.querySelectorAll(tabsSelector),
		tabsContent = document.querySelectorAll(tabsContenctSelector),
	...
	tabsParent.addEventListener('click', function(event) {
		...
		if(target && target.classList.contains(tabsSelector.slise(1))) { // delete first element
            tabs.forEach((item, i) => {
                if (target == item) {
                    hideTabContent();
                    showTabContent(i);
